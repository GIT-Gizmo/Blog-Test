...
app.get("/", (req, res) => {
  res.render("home", { homeContent: contents.home, posts });
});

app.get("/about", (req, res) => {
  res.render("about", { about: contents.about });
});

app.get("/contact", (req, res) => {
  res.render("contact", { contact: contents.contact });
});
...
app.get("/posts/:postId", (req, res) => {
  ...
  const foundPost = posts.find((post) => _.lowerCase(post.title) === postTitle);

  if (foundPost) {
    res.render("post", {
      postPageTitle: foundPost.title,
      postPageContent: foundPost.content,
    });
  } else {
    res.render("404");
  }
